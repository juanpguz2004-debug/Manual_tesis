...
def generar_qr_audio(med, dosis, via, alertas):
    ...
    Solo contiene la información del tratamiento.
    """
    # Construcción del texto seguro
    al_str = ", ".join(alertas) if alertas else "Ninguna"
    
    # Texto redactado para ser leído por la IA
...
def generar_pdf(paciente, farmaceutico, med, dosis, via, frec, alertas, hacer_braille, espejo, hacer_qr):
    ...
    # Fecha y Hora exacta para auditoría
    ...
    # ADVERTENCIA VISUAL DE DOSIS (Requerimiento)
    ...
    # 2. DISCLAIMER LEGAL (FOOTER PÁGINA 1)
    pdf.set_y(-15)
...
with st.container(border=True):
    ...
    if dos:
        c_dos.caption("⚠ Cuidado: Dosis Pediátrica/Adulta.")
